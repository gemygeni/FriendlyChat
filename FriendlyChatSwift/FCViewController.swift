...
import FirebaseUI.FUIGoogleAuth
...
class FCViewController: UIViewController, UINavigationControllerDelegate {
    ...
    func configureAuth() {
        ...
        _authHandle = Auth.auth().addStateDidChangeListener({ (auth, user) in
            ...
            if let activeuser = user{
                //check if the current app user is the current FUIUser
                if self.user != user{
    ...
    func configureDatabase() {
        ...
        _refHandle = ref.child("messages").observe(.childAdded, with: { (snapshot) in
            ...
            self.messagesTable.insertRows(at: [IndexPath(row: (self.messages.count - 1), section: 0)], with: .bottom)
...
extension FCViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let name = message[Constants.MessageFields.name] ?? "name"
